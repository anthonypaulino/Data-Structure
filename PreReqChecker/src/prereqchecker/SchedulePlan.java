package prereqchecker;

import java.util.*;

/**
 * Steps to implement this class main method:
 * 
 * Step 1:
 * AdjListInputFile name is passed through the command line as args[0]
 * Read from AdjListInputFile with the format:
 * 1. a (int): number of courses in the graph
 * 2. a lines, each with 1 course ID
 * 3. b (int): number of edges in the graph
 * 4. b lines, each with a source ID
 * 
 * Step 2:
 * SchedulePlanInputFile name is passed through the command line as args[1]
 * Read from SchedulePlanInputFile with the format:
 * 1. One line containing a course ID
 * 2. c (int): number of courses
 * 3. c lines, each with one course ID
 * 
 * Step 3:
 * SchedulePlanOutputFile name is passed through the command line as args[2]
 * Output to SchedulePlanOutputFile with the format:
 * 1. One line containing an int c, the number of semesters required to take the course
 * 2. c lines, each with space separated course ID's
 */
public class SchedulePlan {
    public static void main(String[] args) {

        if ( args.length < 3 ) {
            StdOut.println("Execute: java -cp bin prereqchecker.SchedulePlan <adjacency list INput file> <schedule plan INput file> <schedule plan OUTput file>");
            return;
        }
    
        //step 1 
        helper PrerequisiteChecker = new helper();

        StdIn.setFile(args[0]);
        
        int numCourses = Integer.parseInt(StdIn.readLine());
        PrerequisiteChecker.makeCourseList(numCourses);
        
        int numPrereqConnections = Integer.parseInt(StdIn.readLine());
        PrerequisiteChecker.makeCoursePrereqList(numPrereqConnections);

        //Step 2
        StdIn.setFile(args[1]);
        String targetCourse = StdIn.readLine();
        int numCoursesTakenSP = Integer.parseInt(StdIn.readLine());
        ArrayList<String> coursesTakenSP = new ArrayList<String>();
        for (int i = 0; i < numCoursesTakenSP; i++) {
            String course = StdIn.readLine();
            coursesTakenSP.add(course);
        }

        ArrayList<String> needToTakeList = PrerequisiteChecker.coursesNeededtoTake(coursesTakenSP,targetCourse);
        HashSet<String> finishedCoursesList = PrerequisiteChecker.getFinishedCoursesList();
        HashMap<Integer, ArrayList<String>> plan = PrerequisiteChecker.makeSchedulePlan(needToTakeList, finishedCoursesList);

        //Step 3
        StdOut.setFile(args[2]);
        
        StdOut.println(plan.size());
        for(Object x: plan.keySet()){
            ArrayList<String> semester = plan.get(x);
            for(String course : semester){
                StdOut.print(course + " ");
            }
            StdOut.println();
        }
    }
}
